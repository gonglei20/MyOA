  ...
  <body>
	...
	<!-- 垃圾邮件列表-->
 					<div class="action">
						...
						<div class="pages">
							<table id="mtb" width="90%" border="0" cellspacing="0" cellpadding="0">
								...
								<c:forEach items="${sessionGarageEmail }" var="emailList">
								<c:if test="${emailList.isgarage ==1 }">
									<tr >
										...
										<td  width="10%">
										<%--  href="reduction.do?id=${emailList.id }" --%>
										<a href="javascript:void(0);" onclick="reduction('${emailList.id }')">还原</a>|
										<a href="javascript:void(0);" onclick="deleteEmailreally('${emailList.id }')">删除</a>
										</td>
									</tr>
									</c:if>
								</c:forEach>
		...
		function reduction(id){
			var con=confirm("确定还原吗?"); //在页面上弹出对话框
			if(con==true){
				location.href = "reduction.do?id="
					+ id;
			}
		}
		function deleteEmailreally(id){
			var con=confirm("确定彻底删除吗?"); //在页面上弹出对话框
			if(con==true){
				location.href = "deleteEmailreally.do?id="
					+ id;
			}
		}
		</script>
